import subprocess
import os
from concurrent.futures import ThreadPoolExecutor
import time
import atexit
import threading
import argparse

running_processes = []
lock = threading.Lock()

def confirm_options():
    parser = argparse.ArgumentParser(description="用于标识是否使用某些组件")
    parser.add_argument('--verbose', action='store_true', help="启用调试模式输出")
    opt = parser.parse_args()
    return opt

def run_external_command(command, cwd=None):
    global running_processes, lock
    lock.acquire()
    try:
        process = subprocess.Popen(
            args=command,
            cwd=cwd,
            stdout=open(os.path.join('logs', f'loop_{len(running_processes)}.log'), 'a', encoding='utf-8'),
            stderr=subprocess.STDOUT,
            text=True,
            encoding='utf-8',
            creationflags=subprocess.CREATE_NEW_PROCESS_GROUP
        )
        running_processes.append(process)
        print(f"非阻塞启动命令: {' '.join(command)} (PID: {process.pid})")
        return process
    except Exception as e:
        print(f"启动命令失败 {' '.join(command)}: {str(e)}")
        return None
    finally:
        lock.release()

def cleanup_processes():
    for process in running_processes:
        if process.poll() is None:
            try:
                process.terminate()
                process.wait(timeout=5)
                print(f"已终止进程 PID: {process.pid}")
            except subprocess.TimeoutExpired:
                process.kill()
                print(f"强制杀死进程 PID: {process.pid}")
            except Exception as e:
                print(f"清理进程 PID:{process.pid} 失败: {str(e)}")

atexit.register(cleanup_processes)

def main():
    opt = confirm_options()
    tasks = [
        {
            'command': ['cmd', '/c', 'npm run dev'],
            'cwd': os.path.join('web')
        },
        {
            'command': ['cmd', '/c', r'conda activate proj && python modules\app.py --verbose'],
            'cwd': '.'
        },
        {
            'command': ['cmd', '/c', r'conda activate proj && python app.py'],
            'cwd': os.path.join('submodules', 'plate')
        },
        {
            'command': ['cmd', '/c', r'conda activate proj && python monitor.py'],
            'cwd': os.path.join('modules')
        },
    ]
    
    with ThreadPoolExecutor(max_workers=6) as executor:
        for task in tasks:
            executor.submit(run_external_command, **task)
    
    print("所有业务任务已后台启动，主程序继续运行...")

if __name__ == "__main__":
    if not os.path.exists('logs'):
        os.makedirs('logs', exist_ok=True)

    mqtt_cwd = os.path.join('submodules', 'mosquitto')
    mosquitto_process = run_external_command(
        command=['mosquitto.exe', '-c', r'mosquitto.conf'],
        cwd=mqtt_cwd
    )

    print("MQTT 服务已后台启动，2秒后启动其他任务...")
    time.sleep(2)

    main()

    try:
        for p in running_processes:
            print(f"PID:{p.pid} 状态: {'运行中' if p.poll() is None else '已退出'}")

        print("所有后台任务已启动，主程序进入循环（按Ctrl+C退出）...")
        while True:
            time.sleep(1)
    except KeyboardInterrupt:
        print("\n接收到退出信号，开始清理...")
    finally:
        cleanup_processes()