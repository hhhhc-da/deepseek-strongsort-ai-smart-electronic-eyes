...
def parse_opt():
    ...
    parser.add_argument('--project', default=os.path.abspath(os.path.join('runs','track')), help='save results to project/name')
...
def track(
        ...
        visualize=False,  # 可视化
        update=False,  # 更新模型
        project=os.path.abspath(os.path.join('runs', 'track')),  # 保存位置
        name='exp',  # 保存名
        exist_ok=False,  # 递增确认
