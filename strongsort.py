...
# python strongsort.py --source rtmp://192.168.43.234:1935/live/114514 --output rtmp://192.168.43.234:1935/live/1919810 --save-vid
    ...
    try:
        ...
        for frame_idx, (path, im, im0s, vid_cap) in enumerate(dataset):
            ...
            for i, det in enumerate(pred):  # detections per image
                    ...
                    if len(outputs[i]) > 0:
                        for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                            ...
                            if True: #lzs save_txt
                                ...
                                with open(os.path.join('runs', 'trace_frames.txt'), 'a') as f: #by lzs added
                ...
                with open(os.path.join("runs", "trace_line_dictionary.txt"), 'w+') as f:
                    f.write(str(tr)) 
...
def main(opt):
    check_requirements(requirements=os.path.join('submodules', 'strongsort', 'requirements.txt'), exclude=('tensorboard', 'thop'))
    run(**vars(opt))
