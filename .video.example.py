...
------------------------- 使用视频推流, 默认使用 YOLOv12 -------------------------
python main.py --source rtmp://www.ayasaki-nanoka.xyz:30004/live/114514 --output rtmp://www.ayasaki-nanoka.xyz:30004/live/1919810 --url http://www.ayasaki-nanoka.xyz:30006 --save-vid

访问 http://www.ayasaki-nanoka.xyz:30005/live?app=live&stream=114514 获取处理前的视频流
访问 http://www.ayasaki-nanoka.xyz:30005/live?app=live&stream=1919810 获取处理后的视频流
...
from modules.track import parse_opt, track
    ...
    if opt.source:
        ...
        if extract_frame(opt.source, output_image_path=cache):
            ...
            if len(dirs) == 0:
                ...
                print(f"YOLO + StrongSort 跟踪后共有 {len(tr)} 个轨迹\n")
                
            # 对 tr 的路径数据进行归一化
            img_w, img_h = img_wh
            data = [[(x / img_w, y / img_h) for (x, y) in tr[i]] for i in tr.keys()]
            
            # 对车辆行为进行计算 (DL: 深度学习, FA: 基于规则的快速分析)
            behavior = predict_behavior_parallel(
                                      data=data,
            ...
            rlpf = request_deepseek(lnpf, qtpf, option=opt)
            ...
            for dc in rst:
                ...
                data = {
                    "security": '98F5F923268E6CDA72ED35CCB930A293',
                    "topic": 'car/abnormal',
                    "text": "车辆{}违规".format(data['plate']),
                    "report": dc['report'],
                    "plate": dc['plate'],
                }
                
                # 发送 MQTT 事件
                requests.post(opt.url + '/publish', json=data)
        else:
