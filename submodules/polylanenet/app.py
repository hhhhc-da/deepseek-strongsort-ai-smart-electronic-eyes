...
# curl -X POST "http://localhost:83/recognize_lane" -F "file=@image\tusimple_sampled_1.jpg"

ROOT = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, ROOT)

CONFIG_YAML_PATH = r'E:\pandownload1\ML\Police\Project\submodules\polylanenet\cfgs\tusimple_fulltrain.yaml'
MODEL_PATH = r'E:\pandownload1\ML\Police\Project\models\model_tusimple_fulltrain.pt'
...
# 车道线识别函数
@app.post("/recognize_lane")
async def recognize_lane(file: UploadFile = File(description="上传需要识别的RGB图片文件")):
    '''
    接收上传的图片文件，返回车道线识别结果
    ...
    try:
        ...
        with torch.no_grad():
            ...
            lanes_data = lane_outputs.cpu().numpy().tolist()
        ...
        for idx, lane in enumerate(lanes_data):
            ...
            try:
                ...
                lower = float(lane[1])
                upper = float(lane[2])
                coeffs = [float(c) for c in lane[3:7]]
            except (ValueError, IndexError):
                continue
...
def test():
    ...
    parser.add_argument("--image", type=str, default=os.path.join(ROOT, 'image', 'tusimple_sampled_1.jpg'), help="One or more image files to process")
